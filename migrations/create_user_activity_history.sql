...
-- This table stores all user activities for the last 15 days (configurable retention period)
...
-- Users can insert their own activity history
CREATE POLICY "Users can insert their own activity history"
  ON user_activity_history
  FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Users can delete their own activity history
CREATE POLICY "Users can delete their own activity history"
  ON user_activity_history
  FOR DELETE
  USING (auth.uid() = user_id);

-- Function to automatically clean up old history (older than retention_days)
